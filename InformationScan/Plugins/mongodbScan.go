...
func mongodbScan(info *config.InfoScan) error {
	if info.Brute == false {
		return nil
	}
	_, err := MongodbUnauth(info)
	if err != nil {
		errlog := fmt.Sprintf("[-] Mongodb unauthorized failed %v:%v %v", info.Hosts, info.Ports, err)
		common.LogError(errlog)
	}
	return err
}

func MongodbUnauth(info *config.InfoScan) (flag bool, err error) {
	flag = false
	// op_msg
	packet1 := []byte{
